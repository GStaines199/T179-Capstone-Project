...
public class RawGnssCaptureTest {
    ...
    @Test
    public void from_withoutBearingOrSpeed_defaultsCoreFieldsToZero() {
        Location location = new Location("gps");
        location.setLatitude(1.0);
        location.setLongitude(1.0);
        // hasBearing()/hasSpeed()/hasAccuracy() are false when never set.

        RawGnssCapture capture = RawGnssCapture.from(location);

        assertEquals(0f, capture.getAccuracyMeters(), 0.0001);
        assertEquals(0f, capture.getBearingDegrees(), 0.0001);
        assertEquals(0f, capture.getSpeedMps(), 0.0001);
    }
}
